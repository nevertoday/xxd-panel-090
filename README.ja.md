<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 090 プロジェクトバナー" width="1200"></p>
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 090 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

物体説明図は「それが何か」しか説明せず、従来型のフローチャートやテック UI は意味を固定テンプレートへ押し込みます。090 が描くのは、写真の背後にある関係、過程、状態、比喩です。

**Panel 090** は写真を概念中心として、動作、方向、空間、感情、関係、比喩を文字、ノード、軌跡、領域、矢印、接続へ分解します。文字の意味体系、幾何学的関係、手作業の痕跡が、絵画・図表・思考ノートの間にある芸術的な地図を作ります。

### 向いている用途

- **写真の背後にある思考過程を表現したい場合：** 動作、方向、関係、感情を、概念中心と視覚的な経路として組み立てます。
- **科学図表ではなく芸術的な図式が必要な場合：** 三角形の構造、折れ線、交差軸、放射線、等高線、場の軌跡を用います。
- **理性と身体性の緊張を求める場合：** 明瞭な幾何学的骨格に、反復する短線、目盛り、記号、点、わずかにずれた手描きの痕跡を重ねます。
- **柔軟な納品が必要な場合：** 上下、左右、デザインのみ、複数比率、4端末向け壁紙に対応し、画像フォルダも一括処理できます。

### すぐ試す

> この画像を XXD Panel 090 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 視覚思考 · 概念マッピング · 図式ドローイング · 実験タイポグラフィ · 視覚記譜 · 手仕事の痕跡
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 写真の関係、過程、状態、暗示を組織した視覚思考マップ |
| **識別しやすい特徴** | 文字ノード、幾何骨格、軌跡、矢印、方向短線、手描きのずれ、大きな余白 |
| **入力の尊重** | 主体、動作、空間、感情、物語関係を概念中心とし、物体説明図にはしない |
| **用途** | アートポスター、表紙、SNS、デザイン単体、複数比率、4種類の端末向け壁紙 |
<!-- xxd-panel-benefit:end -->

## 変換ロジック

```text
主体・輪郭・姿勢・物語上の関係を分析 → 写真を概念中心に据える → 動作・方向・空間・感情・関係・比喩を分解 → 文字・ノード・軌跡・領域・矢印・接続で意味システムを構築 → 三角形・折れ線・交差軸・放射線で幾何学的骨格を組む → 等高線・場の線・波形経路・方向性の mark-making を加える → 少量の文字と広い余白で視覚記譜を完成する
```

## 完成作の特徴

- 幾何線は明晰・鋭利・抑制的にし、手仕事の痕跡は軽いずれ、断裂、不均一を許す。
- 黒、チャコールグレー、または深い混合色を文字と骨格に使い、原写真由来の鮮やかな1–3色をノード、軌跡、密度場に用いる。
- 文字は読めるまま、回転、反復、重なり、経路配置、部分分解でノードや記譜になる。
- 大きな淡色余白を保ち、規則グリッドで全面を埋めない。
- 従来型のマインドマップ、標準的なフローチャート、データダッシュボード、テック UI、サイバーインターフェース、装飾的な乱線、複雑な情報の詰め込み、写実的なイラスト、テンプレート化されたインフォグラフィックを避ける。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-090-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 090 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-090 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-090 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-090 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-090 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 090 を適用し、共通設定は一度だけ解決
/xxd-panel-090 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-090`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-090-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-090-prompt.zh-CN.md)
...
| **[xxd-panel-090](https://github.com/nevertoday/xxd-panel-090)** | 視覚思考 · 概念マッピング · 図式描画 · 実験書体 · 視覚記譜 · 手仕事の痕跡 |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–090） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
