<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 090 프로젝트 배너" width="1200"></p>
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 090이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

사물 설명도는 “무엇인가”만 말하고, 흐름도와 테크 UI는 의미를 고정된 틀로 평평하게 만듭니다. 090은 관계·과정·상태·은유를 그립니다.

**Panel 090**은 사진을 개념 중심으로 보고 동작·방향·공간·감정·관계·은유를 글자·노드·궤적·영역·화살표·연결로 분해합니다. 언어 의미, 기하 관계, 손의 흔적이 그림·도표·사고 노트 사이의 예술 지도를 만듭니다.
...
> 이 이미지를 XXD Panel 090로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 시각적 사고 · 개념 매핑 · 도식 드로잉 · 실험 타이포그래피 · 시각 기보 · 손의 흔적
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 사진의 관계·과정·상태·은유를 조직한 시각적 사고 지도 |
| **대표 특징** | 글자 노드, 기하 골격, 궤적과 화살표, 방향성 짧은 선, 손의 어긋남, 넓은 여백 |
| **입력 존중 방식** | 주제·동작·공간·감정·서사를 개념 중심으로 삼고 사물 설명도로 만들지 않음 |
...
```text
주제·윤곽·자세·서사 관계 분석 → 사진을 개념 중심으로 설정 → 동작·방향·공간·감정·관계·은유 분해 → 글자·노드·궤적·영역·화살표·연결로 의미 체계 구축 → 삼각·꺾은선·교차축·방사선으로 기하 골격 구성 → 등고선·장선·파형 경로·방향성 mark-making 추가 → 소량의 글자와 넓은 여백으로 시각 기보 완성
```
...
- 전통 마인드맵, 표준 흐름도, 데이터 대시보드, 테크 UI, 사이버 인터페이스, 장식적 난선, 복잡한 정보의 과도한 축적, 사실적 일러스트레이션, 템플릿형 인포그래픽을 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-090-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 090의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-090 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-090 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-090 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-090 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-090 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-090 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 090 적용, 공통 설정은 한 번만 확정
/xxd-panel-090 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-090`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-090-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-090-prompt.zh-CN.md)
...
| **[xxd-panel-090](https://github.com/nevertoday/xxd-panel-090)** | 시각적 사고 · 개념 매핑 · 도식 드로잉 · 실험 타이포그래피 · 시각 기보 · 손의 흔적 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–090) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
