# XXD Panel 090 | Runtime Adapter (English)

This is not a second aesthetic prompt. Read `references/original-prompt/zh-CN.md` in full before generation; it is the sole creative and aesthetic authority for Panel 090. If the first Markdown heading is only an internal archival label, omit it from the image request and use the remaining source-brief body verbatim. If there is no such heading, use the entire file. This file only appends the current user's runtime variables.
