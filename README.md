<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 090 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 090 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
分析主体、轮廓、姿态与叙事关系 → 将照片设为概念中心 → 拆解动作、方向、空间、情绪、关系与隐喻 → 用文字、节点、轨迹、区域、箭头和连接建立语义系统 → 以三角、折线、交叉轴线和放射线组织几何骨架 → 加入等高线、场域线、波形路径和方向性 mark-making → 以少量文字和大面积留白完成视觉记谱
```

## 成品中最容易识别的风格特点

- 几何线条清晰、锋利、克制；手工痕迹允许轻微偏移、断裂和不均匀，形成理性结构与身体笔触的张力。
- 用黑、炭灰或深综合色承担文字与骨架，再以原图衍生的 1–3 种鲜活颜色承担节点、轨迹和密度场。
- 文字既可阅读又是图式节点与视觉记谱，允许大小反差、旋转、重复、重叠、沿路径排列和局部拆分。
- 保留大面积浅底留白，让关系、方向和信息之间呼吸；不要用规则网格填满版面。
- 拒绝传统思维导图、标准流程图、数据仪表盘、科技 UI、赛博界面、装饰乱线、写实插画和模板化信息图。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-090-prompt.en.md)
...
| **[xxd-panel-090](https://github.com/nevertoday/xxd-panel-090)** | 视觉思维 · 概念映射 · 图式绘画 · 实验字体 · 视觉记谱 · 手工痕迹 |
...
| **士兵级** | `xxd-panel-NNN`（当前 001–090） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
