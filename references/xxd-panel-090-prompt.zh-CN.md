...
本文件不是第二套美学提示词。生成时必须先完整读取 `references/original-prompt/zh-CN.md`；它是 090 的唯一创作与审美权威。首行 Markdown 标题只是内部归档标签，不进入生图请求；除此之外的原始提示词正文逐字使用。本文件只负责把用户本次选择的运行变量追加到正文末尾。
